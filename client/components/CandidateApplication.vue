<template>
    <form class="container mx-auto flex flex-col w-1/2 bg-white p-10 rounded-3xl shadow-2xl max-lg:w-full max-md:mx-10">
        <h2 class="text-center text-xl font-semibold">Создание заявки</h2>
        <label class="mb-2" for="day">Дата рождения*</label>
        ...
        <HeadlessRadioGroup v-model="citizenship">
            <HeadlessRadioGroupLabel class="">Гражданство*</HeadlessRadioGroupLabel>
            <div class="flex my-5">
        ...
        <input v-if="citizenship === 'other'" class="form-auth-input mb-5" id="day" placeholder="Введите страну">
        <SelectList :content-array="education" v-model="selectedEducation" />
        {{ selectedEducation }}
        <label class="mb-2" for="password">Пароль*</label>
        <input class="form-auth-input mb-5" id="password" type="password" placeholder="Пароль">
        ...
        <div class="flex justify-around max-sm:flex-col max-sm:items-center">
            ...
            <span>Нет аккаунта? <NuxtLink to="/registration" class="font-semibold underline hover:cursor-pointer">
