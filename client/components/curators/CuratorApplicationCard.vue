<template>
    <div class="shadow-slate-500 shadow-lg border-black border rounded-3xl w-full mb-20">
        <div class="p-10">
            ...
            <div class="mb-10 flex gap-x-5">
                <div v-for="(exp, index) of props.application.workExperience" :key="index" class="mb-8">
                    <span class="form-auth-input text-xl">{{ exp.value }}</span>
                </div>
            </div>
            <div class="flex justify-between text-3xl font-bold">
                <button @click="frameApplicationsStore.submitCuratorRespond(application.applicationId!, userStore.user?.userId!, FrameApplicationStatus.APPROVED)">
                    <NuxtImg type="svg" src="/curators/check.svg" />
                </button>
                <button @click="frameApplicationsStore.submitCuratorRespond(application.applicationId!, userStore.user?.userId!, FrameApplicationStatus.DECLINED)">
